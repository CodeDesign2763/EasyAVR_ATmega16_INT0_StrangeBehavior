...
ISR(INT0_vect) {
	...
	PORTA=0x00;
	/* Задержка для лучшей демонстрации
	 * странного поведения */
	_delay_ms(1000);
}

/* Обработчик прерывания INT1 не задан */
...
int main(void)
{
	...
	/* Выводы PD2 и PD3 должны быть подтянут к 0 
	 * внешними средствами */
	
	/* Настройка прерываний */
	/* Разрешение прерываний с векторами INT0 и INT1 */
	/* При этом обработчик INT1 не задан */ 
	GICR |= (1 << INT1) | (1 << INT0);
	
	/* Срабатывание INT0 по спаду импульса */
	MCUCR |= (1 << ISC01) | (0 << ISC00);
	
	/* Срабатывание INT1 по спаду импульса */
	/* MCUCR |= (1 << ISC11) | (0 << ISC10); */
	/* По умолчанию установлено срабатывание INT1 
	 * по низкому уровню (ISC11=0, ISC10=0). 
	 * Из-за этого при J4 в нейтральном положении и SW4=0x00
	 * На PD3 низкий уровень, который постоянно генерирует прерывания.
	 * При этом обработчика прерываний для INT1 нет, что приводит к постоянному
	 * повторному выполнению основной программы. */
